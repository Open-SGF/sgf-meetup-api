...
func NewStack(scope constructs.Construct, id string, props *AppStackProps) awscdk.Stack {
	...
	archivedEventsTable := customconstructs.NewDynamoTable(
		stack,
		props.AppEnv,
	...
	meetupProxyFunction := customconstructs.NewGoLambdaFunction(
		stack,
		jsii.String(meetupProxyFunctionName.Name()),
	...
	//nolint:staticcheck
	meetupProxyFunction.Function.AddToRolePolicy(
	...
	meetupProxyFunctionPolicyNamer := resource.NewNamer(
		props.AppEnv,
		"MeetupProxyFunctionInvokePolicy",
	)
	meetupProxyFunctionInvokePolicy := awsiam.NewManagedPolicy(
		stack,
		jsii.String(meetupProxyFunctionPolicyNamer.Name()),
		&awsiam.ManagedPolicyProps{
			...
			Statements: &[]awsiam.PolicyStatement{
				awsiam.NewPolicyStatement(&awsiam.PolicyStatementProps{
					...
					Resources: jsii.Strings(
						*meetupProxyFunction.Function.FunctionArn(), //nolint:staticcheck
					),
					Effect: awsiam.Effect_ALLOW,
				}),
			},
	...
	awscdk.NewCfnOutput(
		stack,
		jsii.String("MeetupProxyFunctionInvokePolicyArn"),
	...
	meetupGroupNames := awsssm.StringParameter_ValueForStringParameter(
		stack,
		jsii.String("/sgf-meetup-api/meetup-group-names"),
		nil,
	)
	...
	importerFunction := customconstructs.NewGoLambdaFunction(
		stack,
		jsii.String(importerFunctionName.Name()),
		&customconstructs.GoLambdaFunctionProps{
	...
	//nolint:staticcheck
	importerFunction.Function.AddToRolePolicy(
		awsiam.NewPolicyStatement(&awsiam.PolicyStatementProps{
			Effect:  awsiam.Effect_ALLOW,
			Actions: jsii.Strings("ssm:GetParameter", "ssm:GetParametersByPath"),
			Resources: jsii.Strings(
	...
	apiFunction := customconstructs.NewGoLambdaFunction(
		stack,
		jsii.String(apiFunctionName.Name()),
		&customconstructs.GoLambdaFunctionProps{
	...
	importScheduleRule := awsevents.NewRule(
		stack,
		jsii.String("ImporterEventBridgeRule"),
		&awsevents.RuleProps{
			Schedule: awsevents.Schedule_Expression(
				jsii.String("cron(0 0-23/2 * * ? *)"),
			), // every 2 hours
		},
	)
	...
	certificate := awscertificatemanager.NewCertificate(
		stack,
		jsii.String("ApiCert"),
	...
	domain := awsapigateway.NewDomainName(
		stack,
		jsii.String("EventsGatewayDomain"),
		&awsapigateway.DomainNameProps{
			DomainName:   jsii.String(props.DomainName),
	...
	awsapigateway.NewBasePathMapping(
		stack,
		jsii.String("EventsGatewayBasePathMapping"),
		&awsapigateway.BasePathMappingProps{
			DomainName: domain,
			RestApi:    api,
